...
global name_file = "RANDOM-MILP_"*string(TEMPS_SIMU/(3600*inst.n_0))*"h_Nb_Cibles"*string(nbCibles)*".txt"
println(name_file)
open(name_file, "w") do Output
	...
	while (NUM_SATELLITE <= nbMaxSat) && (sum(flag_infeasible[p] for p in 1:NUM_PIXEL) != 0)			
		...
		global Inclinaison = 1.0*pi*rand(NUM_SATELLITE)
		global RAAN = 2.0*pi*rand(NUM_SATELLITE)
		global MeanAnomaly = 2.0*pi*rand(NUM_SATELLITE)
